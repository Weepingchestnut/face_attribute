...
def inception_iccv(pretrained=True, debug=False, **kwargs):
    ...
    """
        ...
        Initializing with baseline models (trained BN-Inception) can obtain better results.
    """
    if pretrained:
...
class SpatialTransformBlock(nn.Module):
    def __init__(self, num_classes, pooling_size, channels):
        ...
        for i in range(self.num_classes):
            ...
            #                                   count_include_pad=True))
            self.gap_list.append(nn.AvgPool2d((pooling_size // 2, pooling_size // 2), stride=1, padding=0, ceil_mode=True,
                                              count_include_pad=True))
    ...
    def forward(self, features):
        ...
        for i in range(self.num_classes):
            ...
            sub_feature = self.stn(stn_feature, theta_i)    # [b, 768, 8, 8]
            # print("sub_feature = {}".format(sub_feature.size()))
...
class InceptionNet(nn.Module):
    def __init__(self, num_classes=51):
        ...
        # psize 64*64
        # self.global_pool = nn.AvgPool2d(2, stride=1, padding=0, ceil_mode=True, count_include_pad=True)
        # psize 128*128
        self.global_pool = nn.AvgPool2d(4, stride=1, padding=0, ceil_mode=True, count_include_pad=True)
        self.finalfc = nn.Linear(1024, num_classes)

        # psize 64*64
        # self.st_3b = SpatialTransformBlock(num_classes, 16, 256 * 3)
        # self.st_4d = SpatialTransformBlock(num_classes, 8, 256 * 2)
        # self.st_5b = SpatialTransformBlock(num_classes, 4, 256)
